"""Hydra fleet tests — 5 workers, lab intelligence, TEE."""
...
with tempfile.TemporaryDirectory() as td:
    ...
    h.record_run("run-1", "researcher-1", model="mimo", tools=["web_search"], skills=["research"], cost_usd=0.1, evaluation_score=0.9, outcome="won", reward_usd=5)
    h.record_run("run-2", "researcher-1", model="mimo", cost_usd=0.1, outcome="lost")
...
# 3. Shared memory (Hydra as authoritative, Letta as derived)
print("\n3. Lab shared memory")
with tempfile.TemporaryDirectory() as td:
    h = HydraStore(f"{td}/hydra.db")
...
with tempfile.TemporaryDirectory() as td:
    ...
    test("candidate promoted", pipe.candidates["pricing omission"].status == "proven")
...
if FAIL: sys.exit(1)
else: print("ALL HYDRA FLEET TESTS PASS")
