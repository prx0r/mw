"""Minimal deterministic evolution lab — gates > scalar fitness."""
...
class Replay:
    def __init__(self, lab: "EvolutionLab"): self.lab=lab
    def run(self, worker_version: dict, tier="hidden")->list[dict]:
        fixtures=self.lab.pack.all_fixtures(tier); out=[]
        for fx in fixtures:
            h=_h({"worker":worker_version,"fixture":fx,"evaluator":_h(self.lab.pack.evaluator_src)})
            score=(int(h[:8],16)%1000)/1000
            out.append({"fixture":fx.get("id",h[:8]),"score":score,"hash":h[:16]})
        return out

@dataclass
class EvolutionLab:
    pack: WorldPack; hydra: HydraStore|None=None; seed: int=0
    def step(self, objective: str, budget: int=8, variants: list[dict]|None=None)->dict:
        ...
        for v in sorted(variants, key=lambda x: _h(x)):
            h=_h({"variant":v,"rubric":self.pack.rubric,"seed":self.seed})
            scores=[(int(_h({"v":v,"fx":fx,"h":h})[:8],16)%1000)/1000 for fx in self.pack.validation or [{"id":"dummy"}]]
            metrics={"mean":sum(scores)/len(scores),"scores":scores,"gates":{g: all(s>0.5 for s in scores) for g in gates}}
            gate_pass=sum(metrics["gates"].values()); metrics["_hash"]=h[:16]
    ...
    def replay(self, worker_version: dict, tier="hidden")->list[dict]: return Replay(self).run(worker_version,tier)
